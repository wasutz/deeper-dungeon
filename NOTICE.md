...
| Asset | Source | Notes |
|---|---|---|
| Cavern ledge terrain and props | Rare Friends Isometric World Assets, through `@rarefriends/friendsdk/world` | The SDK's own renderer and prop set. The scene geometry in `game/world.ts` is original to this game. |
...
| Rare Friend sprites | Canonical Rare Friends Generations sprites, read through `@rarefriends/friendsdk/sprites` from the SDK's pinned artwork deployment | Never rotated, non-integrally scaled, recoloured or replaced. |
| Sound cues | Rare Friends sound kit, through `@rarefriends/friendsdk/sounds` | The SDK's ten cues; see its own `SOUND_KIT.md` and `NOTICE.md`. |
...
Upstream source attribution and content hashes are preserved inside the SDK archive at
`assets/provenance.json` and `assets/sound-provenance.json`.
